...
BOOST_AUTO_TEST_CASE( TEST_WITH_VECTOR )
{
	...
	BOOST_TEST( val.asVector()[ 0 ].which() == ValueT::WHICH_INT );
	BOOST_TEST( val.asVector()[ 0 ].isInt() );
	BOOST_TEST( val.asVector()[ 0 ].asInt() == 10 );
	BOOST_TEST( val.asVector()[ 1 ].which() == ValueT::WHICH_DOUBLE );
	BOOST_TEST( val.asVector()[ 1 ].isDouble() );
	BOOST_TEST( val.asVector()[ 1 ].asDouble() == 20.0 );
}
...
BOOST_AUTO_TEST_CASE( TEST_WITH_MAP )
{
	...
	using MapT		= std::map<std::string, ValueT>;
	...
	BOOST_TEST( val.asMap().at( "intVal" ).which() == ValueT::WHICH_INT );
	BOOST_TEST( val.asMap().at( "intVal" ).isInt() );
	BOOST_TEST( val.asMap().at( "intVal" ).asInt() == 10 );
	BOOST_TEST( val.asMap().at( "strVal" ).which() == ValueT::WHICH_STRING );
	BOOST_TEST( val.asMap().at( "strVal" ).isString() );
	BOOST_TEST( val.asMap().at( "strVal" ).asString() == "string" );
}
